...
local go_case_i = [[
	case func(*ActionCb):
		h.SetCallbackProc("ACTION", syscall.NewCallback(func(PARAMS_GO) int {
...
local go_case_s = [[
	case func(*ActionCb):
		h.SetCallbackProc("ACTION", syscall.NewCallback(func(PARAMS_GO) *C.char {
			arg := &ActionCb{PARAMS_ACT}
			v(arg)
			size := len(arg.Return)
			ch := make([]byte,size+1)
...
func goActionCb(PARAMS_GO) *C.char {
	...
	-- fix
	if ACT == "BUTTON_CB" then
		p_go[#p_go+1] = "noused uintptr"
	end

	local c_tmpl = c_tmpl_i
	local go_tmpl = go_tmpl_i
	local go_case = go_case_i
	if b_change_s == true then
		c_tmpl = c_tmpl_s
		go_tmpl = go_tmpl_s
		go_case = go_case_s
	...
	go_case = string.gsub(go_case,"ACTION",ACT)
	go_case = string.gsub(go_case,"ActionCb",ActCB)
	go_case = string.gsub(go_case,"PARAMS_GO",table.concat(p_go,","))
	go_case = string.gsub(go_case,"PARAMS_ACT",table.concat(p_act,","))	
